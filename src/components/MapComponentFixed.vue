<template>
  <div class="container">
    ...
    <div class="main-content">
      ...
      <div id="map" ref="mapContainer" :style="{ height: mapHeight + 'px' }">
        ...
        <div class="map-controls">
          ...
          <el-button
            type="primary"
            class="control-button work-block-button"
            @click="showWorkBlockDialog">
            作业地块管理
          </el-button>
        </div>
    ...
    <!-- 作业地块管理对话框 -->
    <el-dialog
      v-model="workBlockDialogVisible"
      title="协和成地块作业地块"
      width="1200px"
      :close-on-click-modal="false">
      <div class="work-block-container">
        <div class="work-block-header">
          <el-button type="primary" @click="addWorkBlock">新增</el-button>
          <el-button type="success" @click="exportWorkBlockToExcel">导出Excel</el-button>
        </div>
        
        <el-table :data="workBlocks" border style="width: 100%; margin-top: 15px;">
          <el-table-column prop="id" label="作业行号" width="80" />
          <el-table-column prop="cCarNumber" label="C车编号" width="120" />
          <el-table-column prop="cLastOperationTime" label="C系列最后一次采样时间" width="200" />
          <el-table-column prop="hCarNumber" label="H车编号" width="120" />
          <el-table-column prop="hLastOperationTime" label="H系列最后一次作业时间" width="200" />
          <el-table-column prop="zCarNumber" label="Z车编号" width="120" />
          <el-table-column prop="zLastOperationTime" label="Z系列最后一次作业时间" width="200" />
          <el-table-column prop="pointDistance" label="点间距" width="90" />
          <el-table-column prop="sampleFile" label="采样文件" width="120" />
          ...
          <el-table-column label="操作" width="150" fixed="right">
            <template #default="scope">
              <el-button 
                type="primary" 
                link
                @click="editWorkBlock(scope.row)">
                编辑
              </el-button>
              <el-button 
                type="danger" 
                link
                @click="deleteWorkBlock(scope.row)">
                删除
              </el-button>
            </template>
          </el-table-column>
    ...
    <!-- 作业地块编辑对话框 -->
    <el-dialog
      v-model="workBlockEditDialogVisible"
      :title="workBlockEditForm.id ? '编辑作业地块' : '新增作业地块'"
      width="600px"
      :close-on-click-modal="false">
      <el-form :model="workBlockEditForm" label-width="180px">
        <el-form-item label="作业行号">
          <el-input v-model="workBlockEditForm.id" type="number" :disabled="!!workBlockEditForm.id" />
        </el-form-item>
        <el-form-item label="C车编号">
          <el-input v-model="workBlockEditForm.cCarNumber" />
        </el-form-item>
        <el-form-item label="C系列最后一次采样时间">
          <el-date-picker
            v-model="workBlockEditForm.cLastOperationTime"
            type="datetime"
            placeholder="选择日期时间"
            style="width: 100%;"
          />
        ...
        <el-form-item label="H车编号">
          <el-input v-model="workBlockEditForm.hCarNumber" />
        </el-form-item>
        <el-form-item label="H系列最后一次作业时间">
          <el-date-picker
            v-model="workBlockEditForm.hLastOperationTime"
            type="datetime"
            placeholder="选择日期时间"
            style="width: 100%;"
          />
        ...
        <el-form-item label="Z车编号">
          <el-input v-model="workBlockEditForm.zCarNumber" />
        </el-form-item>
        <el-form-item label="Z系列最后一次作业时间">
          <el-date-picker
            v-model="workBlockEditForm.zLastOperationTime"
            type="datetime"
            placeholder="选择日期时间"
            style="width: 100%;"
          />
        ...
        <el-form-item label="点间距">
          <el-input v-model="workBlockEditForm.pointDistance" type="number" />
        </el-form-item>
        <el-form-item label="采样文件">
          <el-input v-model="workBlockEditForm.sampleFile" />
        </el-form-item>
        <el-form-item label="备注">
          <el-input v-model="workBlockEditForm.remark" type="textarea" :rows="3" />
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="workBlockEditDialogVisible = false">取消</el-button>
          <el-button type="primary" @click="saveWorkBlock">保存</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
export default {
  ...
  setup() {
    ...
    // 添加作业地块管理相关状态
    const workBlockDialogVisible = ref(false)
    const workBlockEditDialogVisible = ref(false)
    const workBlocks = ref([])
    const workBlockEditForm = ref({
      id: null,
      cCarNumber: '',
      cLastOperationTime: null,
      hCarNumber: '',
      hLastOperationTime: null,
      zCarNumber: '',
      zLastOperationTime: null,
      pointDistance: null,
      sampleFile: '',
      remark: ''
    })
    ...
    return {
      ...
      workBlockDialogVisible,
      workBlockEditDialogVisible,
      workBlocks,
      workBlockEditForm,
      showWorkBlockDialog,
      addWorkBlock,
