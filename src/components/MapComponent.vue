...
export default {
  ...
  setup() {
    ...
    const fetchTableRecords = async () => {
      try {
        const response = await axios.get(`http://121.40.132.51:5005/api/tables?page=${currentPage.value}&page_size=${pageSize.value}`)
        if (response.data.success) {
    ...
    const saveTableRecord = async () => {
      try {
        ...
        if (isEditingRecord.value) {
          ...
          response = await axios.put(`http://121.40.132.51:5005/api/tables/${recordForm.value.id}`, record)
        } else {
          // Create new record
          response = await axios.post('http://121.40.132.51:5005/api/tables', record)
        }
    ...
    const handleDeleteTableRecord = (row) => {
      ElMessageBox.confirm(
        ...
        try {
          const response = await axios.delete(`http://121.40.132.51:5005/api/tables/${row.id}`)
          if (response.data.success) {
    ...
    const exportToExcel = async () => {
      try {
        ...
        const response = await axios.get('http://121.40.132.51:5005/api/tables/export')
        if (response.data.success) {
    ...
    const fetchMarkerGroups = async () => {
      try {
        loading.value = true
        const response = await axios.get('http://121.40.132.51:5005/api/marker-groups')
        loading.value = false
    ...
    const handleViewMarkers = async (groupId) => {
      try {
        loading.value = true
        const response = await axios.get(`http://121.40.132.51:5005/api/markers?group_id=${groupId}`)
        loading.value = false
    ...
    const saveMarkerGroup = async () => {
      try {
        ...
        if (isEditingMarkerGroup.value) {
          response = await axios.put(`http://121.40.132.51:5005/api/marker-groups/${markerGroupForm.value.id}`, groupData)
        } else {
          response = await axios.post('http://121.40.132.51:5005/api/marker-groups', groupData)
        }
    ...
    const handleDeleteMarkerGroup = (group) => {
      ElMessageBox.confirm(
        ...
        try {
          loading.value = true
          const response = await axios.delete(`http://121.40.132.51:5005/api/marker-groups/${group.id}`)
          loading.value = false
    ...
    const toggleMarkerGroupVisibility = async (groupId, isVisible) => {
      try {
        const response = await axios.put(`http://121.40.132.51:5005/api/marker-groups/${groupId}/toggle`, {
    ...
    const handleDeleteMarker = (marker) => {
      ElMessageBox.confirm(
        ...
        try {
          loading.value = true
          const response = await axios.delete(`http://121.40.132.51:5005/api/markers/${marker.id}`)
          loading.value = false
    ...
    const saveMarker = async () => {
      try {
        ...
        const response = await axios.post('http://121.40.132.51:5005/api/markers', markerData)
        loading.value = false
    ...
    const fetchAllMarkers = async () => {
      try {
        const response = await axios.get('http://121.40.132.51:5005/api/markers')
    ...
    const addMarkerToMap = (marker, group) => {
      ...
      mapMarker.on('dragend', async () => {
        ...
        try {
          await axios.put(`http://121.40.132.51:5005/api/markers/${marker.id}`, {
