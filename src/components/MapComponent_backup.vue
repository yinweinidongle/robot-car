<template>
  <div class="container">
    ...
    <div class="main-content">
      ...
      <div id="map" ref="mapContainer" :style="{ height: mapHeight + 'px' }">
        ...
        <div class="map-controls">
          ...
          <el-button
            type="primary"
            class="control-button farm-operation-button"
            @click="showFarmOperationDialog">
            作业地块管理
          </el-button>
        </div>
    ...
    <el-dialog
      ...
      title="偏移量设置记录"
      width="800px">
      <offset-history-component 
        :vehicle-id="currentVehicleId" 
        @close="offsetHistoryDialogVisible = false" />
    </el-dialog>
    ...
    <!-- 添加作业地块管理对话框 -->
    <el-dialog
      v-model="farmOperationDialogVisible"
      title="协和成地块作业地块"
      width="90%"
      :close-on-click-modal="false">
      <farm-operation-component />
    </el-dialog>
    
    <!-- 作业地块编辑对话框 -->
    <el-dialog
      v-model="operationFormDialogVisible"
      :title="editingOperationId ? '编辑作业地块记录' : '新增作业地块记录'"
      width="600px"
      :close-on-click-modal="false">
      <el-form 
        :model="operationForm" 
        ...
        <el-form-item label="作业行号" prop="operation_id">
          <el-input v-model="operationForm.operation_id" />
        </el-form-item>
        <el-form-item label="C车编号">
          <el-input v-model="operationForm.c_vehicle_id" />
        </el-form-item>
        <el-form-item label="C系列最后一次采样时间">
          <el-date-picker
            v-model="operationForm.c_last_time"
            type="datetime"
            placeholder="选择日期时间"
        ...
        <el-form-item label="H车编号">
          <el-input v-model="operationForm.h_vehicle_id" />
        </el-form-item>
        <el-form-item label="H系列最后一次作业时间">
          <el-date-picker
            v-model="operationForm.h_last_time"
            type="datetime"
            placeholder="选择日期时间"
            format="YYYY-MM-DD HH:mm:ss"
          />
        ...
        <el-form-item label="Z车编号">
          <el-input v-model="operationForm.z_vehicle_id" />
        </el-form-item>
        ...
        <el-form-item label="点间距">
          <el-input-number v-model="operationForm.distance" :precision="2" :step="0.1" />
        </el-form-item>
        <el-form-item label="点间距采样文件">
          <el-input v-model="operationForm.file_path" />
        </el-form-item>
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="operationFormDialogVisible = false">取消</el-button>
...
import OffsetHistoryComponent from './OffsetHistoryComponent.vue'
import FarmOperationComponent from './FarmOperationComponent.vue'
...
export default {
  ...
  components: {
    ...
    Location,
    Delete,
    OffsetHistoryComponent,
    FarmOperationComponent
  },
  setup() {
    ...
    // 添加作业地块管理相关状态
    const farmOperationDialogVisible = ref(false)
    const operationFormDialogVisible = ref(false)

    // 更新API URL
    const API_URL = 'http://121.40.132.51:5005/api/farm' 

    // 获取作业地块记录
    const fetchOperations = async () => {
      try {
        loading.value = true
        const response = await axios.get(`${API_URL}/operations?page=${operationCurrentPage.value}&page_size=${operationPageSize.value}`)
        
        if (response.data.success) {
          operationRecords.value = response.data.operations.records
          operationTotal.value = response.data.operations.total
        } else {
          ElMessage.error('获取作业地块记录失败：' + response.data.error)
        }
      } catch (error) {
        console.error('Failed to fetch operations:', error)
        ElMessage.error('获取作业地块记录失败：' + error.message)
      } finally {
        loading.value = false
      }
    }
    ...
    const handleDeleteOperation = (row) => {
      ElMessageBox.confirm(
        `确定要删除作业地块记录 "${row.operation_id}" 吗？`,
        '警告',
        {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        ...
        try {
          loading.value = true
          const response = await axios.delete(`${API_URL}/operations/${row.id}`)
          
          if (response.data.success) {
            ElMessage.success('作业地块记录删除成功')
            // 刷新数据表格
            fetchOperations()
          } else {
            ElMessage.error('删除作业地块记录失败：' + response.data.error)
          }
        } catch (error) {
          console.error('Failed to delete operation:', error)
          ElMessage.error('删除作业地块记录失败：' + error.message)
        } finally {
          loading.value = false
        }
      }).catch(() => {
    ...
    const updateOperation = async (operation) => {
      try {
        loading.value = true
        const response = await axios.put(`${API_URL}/operations/${editingOperationId.value}`, operation)
        
        if (response.data.success) {
          ElMessage.success('作业地块记录更新成功')
          // 刷新数据表格
          fetchOperations()
        } else {
          ElMessage.error('更新作业地块记录失败：' + response.data.error)
        }
      } catch (error) {
        console.error('Failed to update operation:', error)
        ElMessage.error('更新作业地块记录失败：' + error.message)
      } finally {
        loading.value = false
      }
    }
    ...
    const addOperation = async (operation) => {
      try {
        ...
        if (response.data.success) {
          ElMessage.success('作业地块记录添加成功')
          // 刷新数据表格
          fetchOperations()
        } else {
          ElMessage.error('添加作业地块记录失败：' + response.data.error)
        }
      } catch (error) {
        console.error('Failed to add operation:', error)
        ElMessage.error('添加作业地块记录失败：' + error.message)
      } finally {
        loading.value = false
      }
    }
    ...
    const handleExportOperations = async () => {
      try {
        loading.value = true
        const response = await axios.get(`${API_URL}/operations/export`)
        
        if (response.data.success) {
          // 将十六进制字符串转换回二进制数据
          const hexString = response.data.data;
          ...
          link.download = '作业地块记录.xlsx';
          link.click();
          
          ElMessage.success('作业地块记录导出成功');
        } else {
          ElMessage.error('导出作业地块记录失败：' + response.data.error);
        }
      } catch (error) {
        console.error('Failed to export operations:', error);
        ElMessage.error('导出作业地块记录失败：' + error.message);
      } finally {
        loading.value = false;
      }
    }
    ...
    const operationFormRules = {
      operation_id: [
        { required: true, message: '请输入作业行号', trigger: 'blur' },
        { pattern: /^[0-9]+$/, message: '请输入有效的数字', trigger: 'blur' }
      ],
      c_vehicle_id: [
        { required: true, message: '请输入C车编号', trigger: 'blur' },
        { pattern: /^[0-9]+$/, message: '请输入有效的数字', trigger: 'blur' }
      ],
      c_last_time: [
        { required: true, message: '请选择C系列最后一次采样时间', trigger: 'change' },
        { type: 'date', message: '请选择有效的日期', trigger: 'change' }
      ],
      h_vehicle_id: [
        { required: true, message: '请输入H车编号', trigger: 'blur' },
        { pattern: /^[0-9]+$/, message: '请输入有效的数字', trigger: 'blur' }
      ],
      h_last_time: [
        { required: true, message: '请选择H系列最后一次作业时间', trigger: 'change' },
        { type: 'date', message: '请选择有效的日期', trigger: 'change' }
      ],
      z_vehicle_id: [
        { required: true, message: '请输入Z车编号', trigger: 'blur' },
        { pattern: /^[0-9]+$/, message: '请输入有效的数字', trigger: 'blur' }
      ],
      distance: [
        { required: true, message: '请输入点间距', trigger: 'blur' },
        { type: 'number', message: '请输入有效的数字', trigger: 'blur' }
      ],
      file_path: [
        { required: true, message: '请输入点间距采样文件', trigger: 'blur' },
        { pattern: /^[a-zA-Z0-9_\-.]+$/, message: '请输入有效的文件名', trigger: 'blur' }
      ],
      notes: [
    ...
    const handleAddOperation = () => {
      operationFormDialogVisible.value = true
      operationForm.value = {
        operation_id: '',
        c_vehicle_id: '',
        c_last_time: '',
    ...
    const handleEditOperation = (row) => {
      editingOperationId.value = row.operation_id
      operationForm.value = {
        operation_id: row.operation_id,
        c_vehicle_id: row.c_vehicle_id,
        c_last_time: row.c_last_time,
        h_vehicle_id: row.h_vehicle_id,
        h_last_time: row.h_last_time,
        z_vehicle_id: row.z_vehicle_id,
        z_last_time: row.z_last_time,
    ...
    const handleDeleteOperation = (row) => {
      ElMessageBox.confirm(
        ...
        try {
          loading.value = true
          const response = await axios.delete(`http://121.40.132.51:5005/api/operations/${row.operation_id}`)
      ...
      operationFormRef.value.validate((valid) => {
        if (valid) {
          if (editingOperationId.value) {
            // 更新现有记录
            updateOperation(operationForm.value)
          } else {
            // 添加新记录
            addOperation(operationForm.value)
          }
          operationFormDialogVisible.value = false
          ElMessage.success('作业地块记录提交成功')
        } else {
    ...
    const updateOperation = async (operation) => {
      try {
        loading.value = true
        const response = await axios.put(`http://121.40.132.51:5005/api/operations/${operation.operation_id}`, operation)
    ...
    const addOperation = async (operation) => {
      try {
        loading.value = true
        const response = await axios.post('http://121.40.132.51:5005/api/operations', operation)
    ...
    const fetchOperations = async () => {
      try {
        loading.value = true
        const response = await axios.get(`http://121.40.132.51:5005/api/operations?page=${operationCurrentPage.value}&page_size=${operationPageSize.value}`)
    ...
    const handleOperationCurrentChange = (page) => {
      operationCurrentPage.value = page
      fetchOperations()
    }
    ...
    const handleExportOperations = async () => {
      try {
        const response = await axios.get('http://121.40.132.51:5005/api/operations/export', {
          responseType: 'blob'
        })
    ...
    // 显示作业地块管理对话框
    const showFarmOperationDialog = () => {
      farmOperationDialogVisible.value = true
      fetchOperations()
    }
